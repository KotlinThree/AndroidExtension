...
class WeakHandler(looper: Looper? = null, val callback: Handler.Callback? = null) {
    ...
    init {
        if (callback == null) {
            mExec = ExecHandler(looper = looper ?: Looper.myLooper())
        } else {
            mExec = ExecHandler(WeakReference(callback), looper = looper ?: Looper.myLooper())
        }
...
private class ChainedRef(var lock: Lock, val runnable: Runnable) {
    var next: ChainedRef? = null
    var prev: ChainedRef? = null
    val wrapper: WeakRunnable

    init {
        this.wrapper = WeakRunnable(WeakReference(runnable), WeakReference(this))
    }
